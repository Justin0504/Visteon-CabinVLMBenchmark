...
MODELS={"base":R+"/models/Qwen2.5-VL-7B-Instruct","v6":R+"/models/bootstrap_v6_merged","v7":R+"/models/bootstrap_v7_merged"}
...
for name,path in MODELS.items():
    ...
    del md; gc.collect(); torch.cuda.empty_cache(); print("answered:",name,flush=True)
...
def sc(img,a):
    r=gen(jm,jp,img,f'A driving assistant answered for this scene: "{a}". Rate 1-10 on factual accuracy of traffic elements, risk analysis quality, and soundness of the driving decision. A good answer reasons scene->risk->decision. Reply ONLY an integer.',8); m=re.search(r"\d+",r); return int(m.group()) if m else None
res={}
...
json.dump(res,open(R+"/data/eval_frozen_v7.json","w"),indent=1); print("EVAL_FROZEN",json.dumps(res)); print("EVAL_FROZEN_DONE")
