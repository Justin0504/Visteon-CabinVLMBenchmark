...
ssh_ "source /home/haiyuez/miniconda3/etc/profile.d/conda.sh; conda activate $SRVDIR/envs/cabin-vlm 2>/dev/null; cd $SRVDIR; VULTR_KEYS=${KEYFILE:-$SRVDIR/.vultr_keys.env} WORKERS=2 VISION_MAX_TOKENS=1500 VISION_MODEL="${VMODEL:-nvidia/Nemotron-3-Nano-Omni-30B-A3B-Reasoning-BF16}" setsid bash -c 'python -m caption_pipeline.multi_extract --inp $DST/input.jsonl --out $DST' > $SRVDIR/logs/harvest_$TAG.log 2>&1 </dev/null & disown; echo LAUNCHED"
